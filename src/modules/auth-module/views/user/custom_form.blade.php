{{ Form::model($item, array('route' => ($newItem ? $group->getRoute("save") : array($group->getRoute("update"), $item->id)), 'files' => true)) }}
    <h1 class="form-title"><?php echo ($newItem ? $group->getLang("title_new") : $group->getLang("title_update")); ?></h1>
    ...
    <div class="panel">
        <div class="panel-body">
            <ul id="employee-nav-tabs" class="nav nav-tabs">
                ...
                <?php app("events")->fire("module.auth.{$group->group_name}.form.tabs.titles", array($item)); ?>
            ...
            <div id="user-tabs" class="tab-content">
                ...
                <?php app("events")->fire("module.auth.{$group->group_name}.form.tabs.panes", array($item)); ?>
                <?php app("events")->fire("module.auth.form.tabs.panes", array($item)); ?>
            </div>
        </div>
    </div>
    <div class="form-actions">
        {{ Form::submit($group->getLang("btn_save"), array('class' => 'btn btn-primary')) }}
        {{ HTML::linkRoute($group->getRoute("list"), $group->getLang("btn_cancel"), null, array("class" => "btn btn-link")) }}
    </div>
{{ Form::close() }}